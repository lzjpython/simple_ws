...
class WebsocketManager:
    ...
    def is_empty(self):
        return bool(self.__conns)

    @property
    def length(self):
        return len(self.__conns)

    def add_websocket(self, socket: WebSocket) -> str:
        socket_id = str(uuid.uuid4())
        self.__conns[socket_id] = socket
        return socket_id

    def remove_websocket(self, socket_id: str):
        del self.__conns[socket_id]

    def get_websocket(self, socket_id: str) -> WebSocket:
        return self.__conns.get(socket_id)
